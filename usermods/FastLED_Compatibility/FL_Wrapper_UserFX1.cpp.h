...
// to generate table:
// - go to https://macetech.github.io/FastLED-XY-Map-Generator/
// - configure tool for your matrix
// - copy XYTable, paste here, give unique name
// - if table has <= 256 values: change uint8_t to uint16_t for consistency
const uint16_t XYTable16x16Serpentine[] = {
...
const uint16_t XYTable32x32Progressive[] = {
...
segmentmap _segmentmaps[MAX_NUM_SEGMENTS] = {
  ...
  { 32, 32, 16*16 , XYTable32x32Progressive}
};
...
uint16_t WS2812FX::writeLedsArrayToWled_XY(CRGB * leds) {
  const uint16_t * xytable = _segmentmaps[_segment_index].mapPtr;
  for (int i=0; i<SEGLEN; i++) {
     setPixelColor(i, leds[xytable[i]].red, leds[xytable[i]].green, leds[xytable[i]].blue);
  }
